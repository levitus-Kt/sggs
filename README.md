...
Также в файле django-server.service в строке ExecStart измените IP 172.23.56.50 на свой. Порт (:8000) можете поменять на свой, если не работает по умолчанию

Файл ringer.service и django-server.service нужно закинуть в папку /etc/systemd/system/ (sudo)

В настройках сети устройства измените IP на статический. Помимо настроек устройства нужно поменять настройки django. В файле mysite/settings.py в строке ALLOWED_HOSTS измените IP 172.23.56.50 на свой (Остальные не трогайте!)
...
Если pip не хочет устанавливать библиотеки, можно руками перенести папку нужной библиотеки в /usr/lib/python3/dist-packages или /usr/local/lib/python{версия-python}/site-packages
...
Управление программой осуществляется через локальный сервер (устройство с программой и устройство, с которого заходите на сервер, должны быть в одной подсети). Вбиваете в адресную строку браузера IP, который вы делали статическим с 8000 портом (н-р 172.23.56.50:8000). Порт (:8000) можете поменять на свой, если не работает по умолчанию
